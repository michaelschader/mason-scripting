...
(defn display [field]
  (dotimes [y (count (aget ^"[[I" field 0))]
    (dotimes [x (count field)]
      (print (aget field x y)))
    (println)))

(defn cell [^sim.field.grid.IntGrid2D agrid ^long x ^long y]
  (aget ^"[I" (aget ^"[[I" (.field ^sim.field.grid.IntGrid2D agrid) (.stx agrid x))
        (.sty agrid y)))

(defn set-cell [^sim.field.grid.IntGrid2D agrid ^long x ^long y value]
  (aset-int ^ints (aget ^objects (.field ^sim.field.grid.IntGrid2D agrid) x) y value))

(defn lcount [^sim.field.grid.IntGrid2D agrid x y] 
  (+ (cell agrid (+ x -1) (+ y -1) ) (cell agrid x (+ y -1)) (cell agrid (+ x 1) (+ y -1))
     (cell agrid (+ x -1) y)         (cell agrid x y)        (cell agrid (+ x 1) y) 
     (cell agrid (+ x -1) (+ y 1))   (cell agrid x (+ y 1))  (cell agrid (+ x 1) (+ y 1))))
                    ...
                    (start []
                      (dotimes [x (count b_heptomino)] 
                        (dotimes [y (count (nth b_heptomino x))] 
                          (aset (.field grid) 
                                (- (+ x (quot (count (.field grid)) 2)) (quot (count b_heptomino) 2))
                                (- (+ y (quot (count (aget (.field grid) x)) 2)) (quot (count (nth b_heptomino x)) 2))
